...
- **The RL agent serves a frozen trained policy in the current deployment:** without real user feedback flowing back at inference time, the agent does not adapt properly after training. Online learning requires a feedback collection pipeline which is yet to be implemented.
